...
AutoComplete.prototype = {
    ...
    pressKey: function (event) {
        ...
        const length = this.autoObj.children.length;
        if (code === 38) {     //↑
            setClass.removeClass(this.autoObj.children[this.index], "on");
            this.index--;
            if (this.index < 0) {
    ...
    start: function (event) {
        const reg = new RegExp("(" + this.obj.value + ")", "i"); //输入"aaa" 则 reg = /(aaa)/i
        ...
        if (code !== 13 && code !== 38 && code !== 40) {
            ...
            const valueArr = this.value_arr.unique();
            //去掉前后空格不能为空
            if (this.obj.value.replace(/(^\s*)|(\s*$)/g, "") === "") {
                return;
            }
            //判断数组中是否含有输入的关键字
            let div_index = 0;  //记录匹配索引个数
...
function processResponse() {
    if (xmlHttpRequest.readyState === 4) {
        if (xmlHttpRequest.status === 200) {
            const resp = xmlHttpRequest.responseText;
            ...
            const autoComplete = new AutoComplete("keyword", "auto", array);
